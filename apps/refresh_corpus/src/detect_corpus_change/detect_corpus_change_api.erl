%%% @doc Cowboy handler — POST /api/rag/corpus/changes/detect.
-module(detect_corpus_change_api).

-export([init/2, routes/0]).

routes() -> [{"/api/rag/corpus/changes/detect", ?MODULE, []}].
...
handle(Req0, _State) ->
    case hecate_rag_http:read_json_body(Req0) of
        {ok, Params, Req1}          -> on_params(detect_corpus_change_v1:from_map(Params), Req1);
        {error, invalid_json, Req1} -> hecate_rag_http:bad_request(<<"Invalid JSON">>, Req1)
    end.

on_params({ok, Cmd}, Req1)       -> reply(maybe_detect_corpus_change:dispatch(Cmd), Req1);
