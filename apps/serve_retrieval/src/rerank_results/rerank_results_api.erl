%%% @doc Cowboy handler — POST /api/rag/queries/rerank.
-module(rerank_results_api).

-export([init/2, routes/0]).

routes() -> [{"/api/rag/queries/rerank", ?MODULE, []}].
...
handle(Req0, _State) ->
    case hecate_rag_http:read_json_body(Req0) of
        {ok, Params, Req1}          -> on_params(rerank_results_v1:from_map(Params), Req1);
        {error, invalid_json, Req1} -> hecate_rag_http:bad_request(<<"Invalid JSON">>, Req1)
    end.

on_params({ok, Cmd}, Req1)       -> reply(maybe_rerank_results:dispatch(Cmd), Req1);
