%%% @doc Cowboy handler — POST /api/rag/knowledge/add.
%%%
%%% Accepts a JSON body with `text` (required), `source_label`, and
%%% `topics` (optional). Runs the full pipeline: chunk → embed → store.
%%% Designed for conversational deposits from agents or interactive
%%% sessions.
-module(add_knowledge_api).

-export([init/2, routes/0]).

routes() -> [{"/api/rag/knowledge/add", ?MODULE, []}].
...
handle(Req0, _State) ->
    case hecate_rag_http:read_json_body(Req0) of
        {ok, Params, Req1}          -> reply(maybe_add_knowledge:add(Params), Req1);
