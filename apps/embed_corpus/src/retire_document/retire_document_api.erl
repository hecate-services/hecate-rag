%%% @doc Cowboy handler — POST /api/rag/documents/retire.
-module(retire_document_api).

-export([init/2, routes/0]).

routes() -> [{"/api/rag/documents/retire", ?MODULE, []}].
...
handle(Req0, _State) ->
    case hecate_rag_http:read_json_body(Req0) of
        {ok, Params, Req1}          -> reply(maybe_retire_document:retire(Params), Req1);
